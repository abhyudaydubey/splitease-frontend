...
const GroupDetailsPage: React.FC = () => {
  ...
  // Fetch group details
  useEffect(() => {
    // Reset state when navigating to a new group
    setLoading(true);
    setError(null);
    setGroup(null);
    ...
    fetchGroupDetails();
  }, [groupId, groupName, navigate, location.pathname]);
  ...
  // Refreshes group details after a new member is added
  const handleMemberAdded = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        toast.error(result.error || 'Failed to refresh group details');
      }
    } catch (error) {
      toast.error('An unexpected error occurred while refreshing group details');
  ...
  const getStatusColor = (status: string | number) => {
    if (status === 'owe' || (typeof status === 'number' && status < 0)) return 'text-gray-700';
    if (status === 'owed' || (typeof status === 'number' && status > 0)) return 'text-gray-800';
    return 'text-gray-600';
  };
  
  // Generate the expense status text
  const getExpenseStatusText = (expense: Expense) => {
    if (!expense.yourStatus) return 'Not involved';
    if (expense.yourStatus === 'owe') return `You owe ${expense.yourShare?.toLocaleString('en-US', { style: 'currency', currency: 'INR' })}`;
    if (expense.yourStatus === 'owed') return `You are owed ${expense.yourShare?.toLocaleString('en-US', { style: 'currency', currency: 'INR' })}`;
    return 'Settled';
  };
  
  // Determine if the expense was paid by the current user
  const isPaidByCurrentUser = (expense: Expense) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Back Button positioned at the far left, aligned with navbar */}
        <div className="relative mb-0">
          ...
          <div className="h-0"></div> {/* Removed spacer height */}
        </div>
        
        {/* Group Information Section - shifted higher up */}
        <div className="bg-white rounded-lg border border-gray-300 shadow-sm mb-4">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-xl font-medium text-gray-900">Group Summary</h2>
          </div>
          ...
          <div className="p-6">
            ...
            <div className="mb-6">
              {group.totalBalance === 0 ? (
                ...
                <div>
                  <div className={`text-lg font-medium ${getStatusColor(group.totalBalance)}`}>
                    {group.balanceText}
                  </div>
                  
                  {/* Individual Balances (Only show if there are any) */}
                    ...
                    <div className="mt-3 text-sm space-y-2">
                      {group.individualBalances.map((balance, index) => (
                        ...
                        >
                          ...
                          <span 
                            className={`font-medium ${getStatusColor(balance.amount)}`}
                          >
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex items-start">
                              ...
                              <div>
                                ...
                                <div className="text-sm text-gray-500">
                                  {isPaidByCurrentUser(expense) ? 'You paid' : `Paid by ${expense.paidBy}`} • {expense.date}
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="text-right">
                              ...
                              {expense.yourStatus && (
                                <div className={`text-sm ${getStatusColor(expense.yourStatus)}`}>
